...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX idx_documents_filename_gin ON documents USING gin (to_tsvector('english', filename))")
    ...
    op.execute("CREATE INDEX idx_region_texts_content_gin ON region_texts USING gin (to_tsvector('english', text_content))")

    # Region Images
    op.create_table('region_images',
    sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('region_id', postgresql.UUID(as_uuid=True), nullable=False),
    ...
    sa.ForeignKeyConstraint(['region_table_id'], ['region_tables.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
